...
`./bin/deploy` builds root-owned app and ingress images, builds Reliquary
component images, then builds child-owned Engram and Synapse images:
...
7. Confirm metadata becomes discoverable through Engram by checking the app
...
8. Confirm movement or reconciliation behavior when Synapse is enabled by
...
tagging/loading, Compose wiring, or runtime startup.
...
Failure category: child packaging | root image tagging/loading | Compose wiring | runtime startup | smoke test
Failed step:
Failed service:
Status snapshot:
