...
As a Mind Palace user, I can open the Settings page from the sidebar and choose a visual theme preset (Mind Palace, Midnight, Warm, or Neutral) so the appearance matches my preference.
...
**Independent Test**: Navigate to Settings via sidebar, select each theme preset card, and verify the entire app UI updates immediately with the corresponding seed color and brightness. Reopen the app and verify the theme persists across restarts.
...
2. **Given** the Settings page is displayed, **When** the page loads, **Then** the user sees a "Settings" heading with sections for Theme Preference and Reset Password.
3. **Given** the Theme Preference section is visible, **When** the user taps the "Mind Palace" preset card, **Then** the entire application UI updates to the Mind Palace theme (purple seed, light brightness) immediately.
4. **Given** the Theme Preference section is visible, **When** the user taps the "Midnight" preset card, **Then** the entire application UI updates to dark mode with the Midnight seed color immediately.
5. **Given** the Theme Preference section is visible, **When** the user taps the "Warm" preset card, **Then** the entire application UI updates to the Warm theme (warm seed, light brightness) immediately.
...
**Independent Test**: Click the "Reset Password" link in the Settings Reset Password section and verify it redirects to the Authentik password reset flow.

**Acceptance Scenarios**:

1. **Given** the Settings page is displayed, **When** the user views the Reset Password section, **Then** a "Reset Password" link or button is visible.
...
- **FR-002**: The Settings page MUST display a "Settings" heading with sections for Theme Preference and Reset Password.
- **FR-003**: The Theme Preference section MUST provide four visual preset cards (Mind Palace, Midnight, Warm, Neutral), each showing a color preview swatch and selecting via a card-based grid layout.
...
- **FR-006**: The Reset Password section MUST display a "Reset Password" link or button that opens the Authentik password reset URL in a new tab.
...
- **ThemeSetting**: A client-side stored enum with four values (`mindPalace`, `midnight`, `warm`, `neutral`), each carrying a `seedColor` and `brightness` that determine the application's visual appearance via Material color generation.
- **ThemeSection**: A named grouping of related settings (Theme Preference, Reset Password) displayed as cards on the Settings page.
